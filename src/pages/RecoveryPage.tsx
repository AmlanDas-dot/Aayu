...
export const RecoveryPage: React.FC = () => {
  ...
  const fetchProfile = async () => {
    try {
      setLoading(true);
      const data = await getRecoveryProfile(userProfile?.uid || 'demo-user');
      setProfile(data);
    } catch (e) {
      console.error(e);
    } finally {
  ...
  const handleCompleteOnboarding = async (formData: any) => {
    await startRecoveryJourney(userProfile?.uid || 'demo-user', formData);
  ...
  const handleRelapse = async () => {
    if (confirm("Are you sure you want to log a relapse? This will gently reduce your score and reset your streak, but your history is preserved. We are here to support you!")) {
      await logRelapse(userProfile?.uid || 'demo-user');
      fetchProfile();
    }
  };
  ...
  if (loading) {
    return <div style={{ padding: '40px', textAlign: 'center' }}>Loading behavioral health profile...</div>;
  }
  ...
  return (
    <div className="recovery-dashboard" style={{ padding: '24px', background: '#f8fafc', minHeight: '100vh', display: 'flex', flexDirection: 'column', gap: '24px' }}>
      ...
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
        <div>
          <h1 style={{ margin: 0, fontSize: '28px', color: '#0f172a', fontWeight: 'bold' }}>Behavioral Health & Recovery</h1>
