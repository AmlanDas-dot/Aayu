...
def test_phase9_complex_phrase_is_split_into_patient_safe_candidates() -> None:
    candidates, issues = build_question_candidates(
...
def test_phase9_question_selection_avoids_fragment_questions() -> None:
    top_diseases = [
        {
            "id": "skin_diseases__ringworm",
            "clinical_score": 0.88,
            "question_candidates": "a ring-shaped rash || a rash with a lighter center and a ring around it || itching",
            "raw_symptoms": "annular, scaly, erythematous plaque with central clearing || itching",
            "tags": "",
            "title": "Ringworm",
        },
        {
            "id": "skin_diseases__granuloma_annulare",
            "clinical_score": 0.82,
            "question_candidates": "a ring-shaped rash || redness",
            "raw_symptoms": "ring-shaped, skin-coloured or red papules || central clearing",
            "tags": "",
            "title": "Granuloma Annulare",
        },
        {
            "id": "skin_diseases__eczema",
            "clinical_score": 0.79,
            "question_candidates": "itching || peeling skin || redness",
            "raw_symptoms": "itchy, scaly rash",
            "tags": "",
            "title": "Eczema",
        },
    ]

    next_symptom = select_next_question(
        top_diseases,
        asked_symptoms=["ring_shaped_rash", "itching", "redness"],
        denied_symptoms=[],
    )
    ...
    _, _, metadata = _build_doc(
        {
            ...
            "guidance": "Keep the area dry.",
            "precautions": ["Do not share towels."],
            "urgency": "low",
        },
        "skin_diseases",
    )
...
def test_phase9_generated_questions_score_well_and_final_response_is_present() -> None:
    candidates, _ = build_question_candidates(
    ...
    payload = start_screening(session_id, ["joint_pain"], {"age": 44, "gender": "male"})
